...
void testCodeGeneration(
...
}) => test('$builder - generates correct code for $assetPrefix.dart', () async {
  ...
  await testBuilder(builder, reader: reader, writer: writer, {
    'a|$assetPrefix.dart': sourceContent,
  });

  expect(writer.assets, hasLength(1));
  final MapEntry(key: id, value: binaryContent) = writer.assets.entries.single;
  expect(id, assetId);
  final decodedContent = utf8.decode(binaryContent);

  printOnFailure(decodedContent);

  return resolveSource(inputId: assetId, decodedContent, (resolver) async {
